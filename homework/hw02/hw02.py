...
def get_the_cake(missions):
    ...
    ['Expr', 'Return']
    """
    return missions(lambda x: x * 2)(lambda y: y)(0)(1)(2)(3)(4)
...
def church_to_int(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    return n(lambda x: x + 1)(0)  #f为lambda x: x + 1，注意到进行f操作的次数为数字，对0递增f次为结果
...
def add_church(m, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    return lambda f: lambda x: m(f)(n(f)(x))  #向内调用得到一个结果塞入另一个x
...
def mul_church(m, n):
    ...
    """
    "*** YOUR CODE HERE ***"
    return lambda f: m(n(f))  #将n中lambda传入m，将每一个f等同为n的操作
