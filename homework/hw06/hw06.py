...
class shiftcipher(methods):
    ...
    def encrypt(self, message):
        ...
        output = ''
        for i in message:
            after_shift = ord(i) + self.shift
            if after_shift > ascii_z:
                after_shift = (after_shift - ascii_z) % 26 + ascii_a - 1
            output += chr(after_shift)
        return output
...
class dictionarycipher(methods):
    ...
    def encrypt(self, message):
        ...
        return ''.join([(self.dictionary.get(i, i)) for i in message])
...
class fencecipher(methods):
    ...
    def encrypt(self, message):
        ...
        for i in range(self.rails):
            for j in range(i, len(message), self.rails):
                output += message[j]
        return output
...
class splitencryption(extensions):
    ...
    def decorator(self, function, message):
        ...
        if self.x == 0:
            ...
            select = []
            for i in range(self.x, len(message), self.x):
                select.append(message[i])
                message[i] = ''
            message += select
        output = ''.join(message)
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for one_card in opponent.deck.cards[:]:
            if (one_card.attack == other_card.attack) or (one_card.defense == other_card.defense):
                opponent.deck.cards.remove(one_card)
