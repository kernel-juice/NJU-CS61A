...
def cumulative_mul(t):
    ...
    def mymul(self, other):
        ...
        for branch in self.branches:
            other = mymul(branch, other)
        return other

    if isinstance(t, Tree):
        for branch in t.branches:
            t.label = mymul(branch, t.label)
...
def prune_small(t, n):
    ...
    if isinstance(t, Tree):
        ...
        for branch in t.branches:
            ...
            all_label.append(branch.label)
        while len(t.branches) > n:
            t.branches.pop(all_label.index(max(all_label)))
            all_label.remove(max(all_label))
        for branch in t.branches:
