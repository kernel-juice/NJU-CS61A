...
def do_if_form(expressions, env):
    ...
    if is_scheme_true(scheme_eval(expressions.first, env)):
        ...
        if expressions.rest.rest.rest is nil:
...
def make_let_frame(bindings, env):
    ...
    now = bindings.first
    validate_form(now, 2, 2)
    names = Pair(now.first, nil)
    values = Pair(scheme_eval(now.rest.first, env), nil)
    bindings = bindings.rest

    def loop(names, values, bindings):
        if bindings is nil:
            return

        now = bindings.first
        validate_form(now, 2, 2)
        names.rest = Pair(now.first, nil)
        values.rest = Pair(scheme_eval(now.rest.first, env), nil)
        loop(names.rest, values.rest, bindings.rest)
    # END PROBLEM 14

    loop(names, values, bindings)
